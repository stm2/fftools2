 ...
 * @version
 */
public class SoldatenFrontAction extends MenuAction {
	 ...
	 */
	public SoldatenFrontAction(SelectionObserver selectionObserver) {
        super(selectionObserver.getClient());
        setName("Temp: Soldaten Front");
	...
	private void makeIt(Unit u){
		...
		tempUnit.addOrderAt(0, "// script Lernfix Lernplan=ArmeeHiebwaffen");
		tempUnit.addOrderAt(0, "// script Material");
		
		// rekrutieren
		tempUnit.addOrderAt(0, "// script Runde " + this.selectionObserver.getClient().getData().getDate().getDate() + " script Rekrutieren 10", false);
